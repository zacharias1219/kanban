...
function ColumnContainer(props: Props) {
    ...
    if (isDragging) {
      return <div ref={setNodeRef} style={style} className='bg-columnBackgroundColor opacity border-2 border-rose-500 w-[350px] h-[500px] max-h-[500px] rounded-md flex flex-col'></div>;
    }
    ...
    <div ref={setNodeRef} style={style} className='bg-columnBackgroundColor w-[350px] h-[500px] max-h-[500px] rounded-md flex flex-col'>
        ...
        <div {...attributes} {...listeners} onClick={() => {setEditMode(true);}} className='bg-mainBackgroundColor text-md h-[60px] cursor-grab rounded-md rounded-b-none p-3 font-bold border-columnBackgroundColor border-4 flex items-center justify-between'>
          <div className='flex gap-2'>
            <div className='flex justify-center rounded-full items-center bg-columnBackgroundColor px-2 py-1 text-sm'>
              0
            </div>
            {column.title}
            {!editMode && column.title}
            {editMode && <input className='bg-black focus:border-rose-500 border-rounded outline-none px-2' value={column.title} onChange={e => updateColumn(column.id, e.target.value)} autoFocus onBlur={() => { setEditMode(false);}} onKeyDown={e => {
              if (e.key === 'Enter') return;
              setEditMode(false);
            }}/>}
          </div>
        </div>
        <button onClick={() => deleteColumn(column.id)} className='stroke-grey-500 hover:stroke-white hover:bg-columnBackgroundColor rounded px-1 py-2'>
          <TrashIcon />
        </button>
